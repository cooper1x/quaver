(function(){
    ...
    Game.prototype.start = function(){
        ...
        var startX = 0;
        for(var i = 0; i < 20; i++){
            var randomWidth = _.random(500,800);
            new Map(startX,game.canvas.height * 0.7,randomWidth,gap);
        ...
        start().then(function(analyser){
            self.timer = setInterval(function(){
                ...
                for (var i = 0; i < game.mapArr.length; i++) {
                    game.mapArr[i] && game.mapArr[i].update(game.r.isMove,voiceSize);
                    game.mapArr[i] && game.mapArr[i].render();
                }
                // self.m.update(game.r.isMove);
                // self.m.render();
                //更新渲染角色
                self.r.update(voiceSize);
                self.r.render(voiceSize);
                //打印帧编号
                self.ctx.font = "16px consolas";
                self.ctx.textAlign = "left";
                self.ctx.fillStyle = "black";
                self.ctx.fillText("FNO:" + self.fno , 10 ,20);
                self.ctx.fillText("Score:" + self.score , 10 ,37);
                // console.log("hah");
            }, 40)
        })
    }
